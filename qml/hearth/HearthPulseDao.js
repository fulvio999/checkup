   ...
   */
    ...
    */
    function getYaxisWeigthValues(dateFrom, dateTo){
         ...
         var rs = "";
           db.transaction(function(tx) {
              rs = tx.executeSql("select value from hearth_pulse b where date(b.date) <= date('"+fullDateTo+"') and date(b.date) >= date('"+fullDateFrom+"')");
            }
         );
    ...
    */
    function getXaxis(dateFrom, dateTo){
        ...
        var rs = "";
        db.transaction(function(tx) {
            rs = tx.executeSql("select date from hearth_pulse b where date(b.date) <= date('"+fullDateTo+"') and date(b.date) >= date('"+fullDateFrom+"')");
          }
        );
