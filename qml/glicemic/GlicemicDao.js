   ...
   */
    ...
    */
    function getYaxisWeigthValues(dateFrom, dateTo){
         ...
         var rs = "";
           db.transaction(function(tx) {
              rs = tx.executeSql("select value from glicemic b where date(b.date) <= date('"+fullDateTo+"') and date(b.date) >= date('"+fullDateFrom+"') order by b.date asc");
            }
         );
    ...
    */
    function getXaxis(dateFrom, dateTo){
        ...
        var rs = "";
        db.transaction(function(tx) {
            rs = tx.executeSql("select date from glicemic b where date(b.date) <= date('"+fullDateTo+"') and date(b.date) >= date('"+fullDateFrom+"') order by b.date asc");
          }
        );
